<tr class="transition hover:bg-[#142a28]/60">
    ...
    <td class="px-4 py-3">
        ...
        @if ($customer->address)
            <div class="text-sm text-[#a9c2bd]">{{ $customer->address }}</div>
        @endif
    </td>
    <td class="px-4 py-3">
        <span class="text-[#a9c2bd]">{{ $customer->customerType->name ?? __('No type') }}</span>
    </td>
    <td class="px-4 py-3">
    ...
    <td class="px-4 py-3">
        <div class="flex flex-wrap items-center justify-end gap-2">
            <flux:button type="button" size="sm" variant="ghost" wire:click="startEditing({{ $customer->id }})"
                wire:loading.attr="disabled" wire:target="startEditing">
                {{ __('Edit') }}
            </flux:button>

            <flux:button type="button" size="sm" variant="danger" wire:click="confirmDelete({{ $customer->id }})"
